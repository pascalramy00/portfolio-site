...
const Project = ({
  project: { title, description, story, date, url, videoUri, stack, features },
}: {
  project: Project;
}) => {
  return (
    <li className="flex flex-col md:w-full smallDesktop:w-full smallDesktop:flex-col lg:w-full lg:flex-row mt-10 mb-16">
      {/* Left Column: Video */}
      ...
      <div className="w-full smallDesktop:w-full lg:w-1/2 flex flex-col justify-between px-0 smallDesktop:px-0 smallDesktop:mt-2 smallDesktop:justify-between mt-2 lg:mt-0 lg:px-10">
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="block w-1.5 h-1.5 bg-foreground rounded-full"></span>
            <ExternalLink href={url} label="Visit" className="my-1 text-sm" />
          </div>
          <p className="mb-2 text-sm">{story}</p>
        </div>
        ...
        <div>
          ...
          {features && (
            <FeatureBanner features={features} className="text-sm" />
          )}
        </div>
      </div>
    </li>
